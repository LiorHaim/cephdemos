...
Now we'll generate the dockercoins-operator manifests using operator-sdk command and the the directory context: 
```bash
...
This example shows that the main.yaml playbook that is called by the ansible-runner, uses the K8S ansible module to interact with out Openshift cluster. this task looks for a Jinja template located in the templates dir, This template looks as the following: 
...
We could also specify ansible extra-vars (such as redis_replicas) that will be transfered through the CR spec section, and the playbook will render those vars into the playbook. We'll see how this happens later on. To sum things up, the basic architecture of the operator login is: 

Operator --> watchers.yaml --> ansible-runner --> ansible role --> ansible K8S module --> resource creation --> Acknowledge 

After we understand far more how things work, let's move on with the demo, the next thing we want to do is to create the CRD that will be used as the CR template:
```bash 
...
We have used the operator-sdk command to build our operator container image including a tag, now we can push it to a container registry (in our case, Docker Hub). Notice that for disconnected container registries you need the add some extra configuration (add pull secrets and link the to the right service account).              
...
After the push was successful, we need to replcate the operator image name in the operator deployment:
```bash
...
Let's continue with the creation of RBAC roles, service account, and security constraints:
...
These manifest are crucial, we need them the allow the user that was created by the operator-sdk to interact with our Openshift cluster. 
Now we can deploy out operator: 
...
So we see that we out operator running and waiting for our CR creation, before we create it let's take a look at the CR manifest: 
...
So our default CR look like that, let's change it a bit to understand how we could use ansible extra-vars to create greater flexabillity. Previous in this demo we saw that in the Jinja template we have a variable definition `{{ redis_replicas|default(size) }}` This definition says that in case we have redis_replicas variable specified we use it, or else we will fetch it from out default variables in the default directory. There is no redis_replicas variable neither in vars nor defaults directories because we will render those vars to ansible using the CR's spec section. The Ce should look like this: 
...
As you see, we have used our own attributes to create the `dockercoins` cluster, so we can actually control which variables can be changed in our ansible role. This CR will create a cluste where we use the default size which is 3 to the hasher and the rng services, but the rest of the services will get their replica size using the following CR. Now we can create the CR and see if those vars were successfuly rendered to the ansible-runner:
...
We see we have 2 replicas in Terminating which means the service scaled down: 
